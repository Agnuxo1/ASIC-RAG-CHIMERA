#!/usr/bin/env python3
"""
ASIC-RAG-HEALTH: Cryptographically Verified Medical Record Sealing v10.0

CRITICAL V10 FIX: VERSION ROLLING Correction based on ESP-Miner

ESP-Miner log analysis revealed the core issue:
  - The ASIC returns: Ver: 06E42000 (only modified bits)
  - ESP-Miner internally calculates: Ver: 26E42000 (0x20000000 | 0x06E42000)
  - Mining.submit sends: "06e42000" (without the base)

To verify correctly, we must bitwise OR with 0x20000000 (BIP9 base).
...
class Config:
    ...
    # Difficulty for ~10-15s per share on LV06
    DIFFICULTY = 0.08
    ...
    # Output
    OUTPUT_DIR = Path("chimera_v10_verified")
...
class StratumBridgeV6(threading.Thread):
    ...
    def verify_share(self, job: Dict, submit: Dict) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            # VERSION ROLLING FIX (V10):
            # Evidence from ESP-Miner logs shows ASIC sends version_bits WITHOUT base 0x20000000:
            #   bm1368Module: Ver: 06E42000   <- ASIC returns this
            #   asic_result: Ver: 26E42000    <- After OR: 0x20000000 | 0x06E42000
            #   Submit sends: "06e42000"      <- What we receive
            #
            # The ASIC modifies bits within the version_rolling_mask, but the final
            # version MUST have the BIP9 base (0x20000000) ORed back in.
            ...
            if submit.get('version_bits'):
                ...
                # Smart detection: if bit 29 is already set, ASIC sent full version
                # Otherwise, we need to OR with base
                if version_bits & BASE_VERSION:
                    # Bit 29 already set - use as-is (some ASICs send full version)
                    version_used = version_bits
                    if Config.DEBUG:
                        print(f"[DEBUG] Version (full): {submit['version_bits']} -> {format(version_used, '08x')}")
                else:
                    # Bit 29 not set - need to OR with base (standard BM1366 behavior)
                    version_used = BASE_VERSION | version_bits
                    if Config.DEBUG:
                        print(f"[DEBUG] Version (OR): {submit['version_bits']} | 0x20000000 -> {format(version_used, '08x')}")
            else:
            ...
            header = build_block_header(
                version=version_used,  # USE THE VERSION FROM ASIC!
