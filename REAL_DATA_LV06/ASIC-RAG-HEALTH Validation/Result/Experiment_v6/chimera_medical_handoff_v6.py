#!/usr/bin/env python3
"""
ASIC-RAG-HEALTH: Cryptographically Verified Medical Record Sealing v6.0

FIX CRÍTICO: Soporte para VERSION ROLLING (BIP 320)

El LV06 (BM1366) usa version-rolling para aumentar el espacio de nonces.
En lugar de solo modificar el nonce de 32 bits, también modifica bits
de la versión del bloque.

El mining.submit con version-rolling tiene 6 parámetros:
  [worker, job_id, extranonce2, ntime, nonce, version_bits]

Debemos usar version_bits (no la versión original) para reconstruir
...
class Config:
    ...
    # Output
    OUTPUT_DIR = Path("chimera_v6_verified")
