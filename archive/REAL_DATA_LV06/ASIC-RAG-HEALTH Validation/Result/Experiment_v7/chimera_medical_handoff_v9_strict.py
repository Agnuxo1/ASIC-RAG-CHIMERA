...
class Config:
    ...
    # Difficulty for shares - lowered for testing
    DIFFICULTY = 1
    ...
    # Output
    OUTPUT_DIR = Path("chimera_v9_strict")
...
# =============================================================================
# PYMINER HELPER FUNCTIONS (Strict BM1366 Verification)
# =============================================================================
...
def swap_endian_word(hex_word):
    ...
    # Pad to 4 bytes if needed, though usually it should be 4 bytes
    if len(message) < 4:
        message = message.rjust(4, b'\x00')
    if len(message) != 4: raise ValueError(f'Must be 4-byte word: {hex_word}')
    return message[::-1]
...
def swap_endian_words(hex_words):
    ...
    if len(message) % 4 != 0:
        raise ValueError(f'Must be 4-byte word aligned: {hex_words}')
...
def reverse_bytes_pyminer(data):
    data = bytearray(data)
    len_data = len(data)
...
def hex_to_be(hex_str):
    bin_be = swap_endian_words(hex_str)
    bin_be = reverse_bytes_pyminer(bin_be)
    return bin_be
...
class StratumBridgeV6(threading.Thread):
    ...
    def verify_share(self, job: Dict, submit: Dict) -> Tuple[bool, str, Dict]:
        """
        Verify share using STRICT PYMINER logic for BM1366.
        """
        verification = {}
        ...
        try:
            ...
            # 2. Compute Merkle Root (Standard SHA256d up the tree)
            merkle_root_bin = coinbase_hash
            for branch in job['merkle_branches']:
                merkle_root_bin = sha256d(merkle_root_bin + hex_to_bytes(branch))
            
            # 3. Apply PyMiner's Merkle Root Transformation
            # PyMiner line 121: binascii.hexlify(swap_endian_words(binascii.hexlify(self._merkle_root_bin).decode('utf8'))).decode('utf8')
            merkle_root_hex = bytes_to_hex(merkle_root_bin)
            merkle_root_swapped = bytes_to_hex(swap_endian_words(merkle_root_hex))
            verification['block_merkle_root'] = merkle_root_hex
            verification['merkle_root_swapped'] = merkle_root_swapped # The one used in header
            
            # 4. Prepare Header components
            
            # Version
            # PyMiner logic: header = swap_endian_word(int_to_hex32(int(job._version, 16) ^ int(result['version'], 16)))
            # result['version'] comes from ASIC. If Version Rolling is on, it might be just the rolled bits?
            # In V7 we assumed submit['version_bits'] was the full version.
            # Let's assume submit.get('version_bits') is what we need to XOR or use.
            
            job_version_int = int(job['version'], 16) if isinstance(job['version'], str) else job['version']
            
            if submit.get('version_bits'):
                 asic_version_int = int(submit['version_bits'], 16)
                 # V7 logic was: (job['version'] & ~mask) | (version_bits & mask)
                 # PyMiner uses XOR. Let's try PyMiner's XOR if we assume asic_version_int is the DIFF.
                 # BUT Stratum usually sends the FULL version in version_bits (if mask is ffffffff).
                 # Let's stick to V7's version reconstruction which we know is "correct" for Stratum standard,
                 # but then use PyMiner's swap_endian_word on it.
                 version_used = (job_version_int & ~self.version_rolling_mask) | (asic_version_int & self.version_rolling_mask)
            else:
                 version_used = job_version_int
            
            verification['version_used'] = version_used
            
            # 5. Build Header (PyMiner Style)
            # header = swap_endian_word(version) + swap_endian_words(prevhash) + swap_endian_words(merkle) + ...
            
            # Version
            header = swap_endian_word(int_to_hex32(version_used))
            
            # Prevhash (job['prevhash_stratum'] is ALREADY 32-bit swapped by Stratum standard?)
            # PyMiner takes job._prevhash. In its Job dict it looks like standard hex.
            # IN V7, job['prevhash_stratum'] is what we sent to miner. 
            # PyMiner: header += swap_endian_words(job._prevhash)
            # We need the RAW prevhash (not stratum swapped) to apply swap_endian_words?
            # Actually, swap_endian_words IS the stratum swap.
            # So if job['prevhash_stratum'] is already swapped, we convert to bytes and use it?
            # Wait, PyMiner does `swap_endian_words` on the hex string.
            # If `job['prevhash_stratum']` is "aabbcc...", `hex_to_bytes` gives bytes.
            # Let's use `job['prevhash_raw']` which is the original BE/LE hash?
            # In V7, `prevhash_stratum` = `swap_endianness_32bit_words(medical_merkle_root)`.
            # So `medical_merkle_root` is our "prevhash". 
            # PyMiner does `swap_endian_words(job._prevhash)`.
            # So we should take `medical_merkle_root` (hex) and apply `swap_endian_words` (bytes).
            header += swap_endian_words(job['prevhash_raw']) # Use Raw!
            
            # Merkle Root
            # PyMiner uses `job._merkle_root` which was `merkle_root_swapped`.
            # And then `verify_work` calls `swap_endian_words(job._merkle_root)`.
            # This SWAPS IT BACK to original? Or swaps again?
            # If `job._merkle_root` is hex string of swapped words.
            # Then `swap_endian_words` on THAT string produces bytes of... double swapped?
            # Let's follow PyMiner.
            # Job.__init__ stores `_merkle_root`.
            # verify_work uses `job._merkle_root`.
            # So we use `merkle_root_swapped` (hex) and swap it again?
            header += swap_endian_words(merkle_root_swapped)
            
            # NTime
            header += swap_endian_words(submit['ntime'])
            
            # NBits
            header += swap_endian_words(job['nbits_hex'])
            
            # Nonce
            header += swap_endian_words(submit['nonce'])
            
            verification['block_header_hex'] = bytes_to_hex(header)
            
            # 6. Verify (PyMiner Style)
            ...
            # We use job['target'] which is int. Convert to hex256.
            target_hex = int_to_hex256(job['target'])
            
            hash_buffer = hashlib.sha256(header).digest()
            hash_result = hashlib.sha256(hash_buffer).digest()
            hash_str = bytes_to_hex(hash_result)
            
            hash_be = swap_endianness_32bit(hex_to_be(hash_str))
            final_hash_str = bytes_to_hex(hash_be)
            
            verification['block_hash_hex'] = final_hash_str
            
            if final_hash_str < target_hex:
                return (True, "", verification)
            else:
                # Calculate ratio for info
                hash_int = int(final_hash_str, 16)
                target_int = int(target_hex, 16)
                ratio = hash_int / target_int if target_int > 0 else 0
