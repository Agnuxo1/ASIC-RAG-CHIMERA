...
def run_rerc_validation(mode="normal"):
    ...
    for t in range(len(u)):
        ...
        X.append(state)
        Y.append(y[t])
        telemetry.append({"t": t, "input": u[t] if mode != "constant" else 0.25, "shares": n, "nrmse_mode": mode})
...
if __name__ == "__main__":
    ...
    for k, v in results.items():
        nrmse_val, mode_telemetry = v
        print(f" {k.upper():10}: NRMSE = {nrmse_val:.6f}")
        all_telemetry.extend(mode_telemetry)
        results[k] = nrmse_val # Flatten for JSON
    print("="*40)
    
    # SAVE RESULTS
    with open("rerc_rigor_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    import csv
    keys = all_telemetry[0].keys()
    with open("rerc_telemetry.csv", "w", newline="") as f:
        dict_writer = csv.DictWriter(f, fieldnames=keys)
        dict_writer.writeheader()
        dict_writer.writerows(all_telemetry)
    
    print(f"\n[OK] Results saved to rerc_rigor_results.json and rerc_telemetry.csv")
